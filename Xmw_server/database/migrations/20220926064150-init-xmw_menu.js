 ...
 * @LastEditors: Cyan
 * @LastEditTime: 2022-09-26 18:31:08
 */
'use strict';
...
module.exports = {
  async up (queryInterface, Sequelize) {
    ...
    await queryInterface.createTable('xmw_menu', {
      ...
      path: { type: STRING(100), allowNull: false, comment: '路由url' },
      ...
      layout: { type: STRING(10), allowNull: false, comment: '是否显示layout布局' },
      hideChildrenInMenu: { type: STRING(10), allowNull: false, comment: '是否隐藏子路由' },
      hideInMenu: { type: STRING(10), allowNull: false, comment: '是否隐藏菜单，包括子路由' },
      hideInBreadcrumb: { type: STRING(10), allowNull: false, comment: '是否在面包屑中显示' },
      headerRender: { type: STRING(10), allowNull: false, comment: '是否显示顶栏' },
      footerRender: { type: STRING(10), allowNull: false, comment: '是否显示页脚' },
      menuRender: { type: STRING(10), allowNull: false, comment: '当前路由是否展示菜单' },
      menuHeaderRender: { type: STRING(10), allowNull: false, comment: '当前路由是否展示菜单顶栏' },
      flatMenu: { type: STRING(10), allowNull: false, comment: '子项往上提，只是不展示父菜单' },
      fixedHeader: { type: STRING(10), allowNull: false, comment: '固定顶栏' },
      fixSiderbar: { type: STRING(10), allowNull: false, comment: '固定菜单' },
      navTheme: { type: STRING(10), allowNull: false, comment: '导航菜单的主题' },
      headerTheme: { type: STRING(10), allowNull: false, comment: '顶部导航的主题，mix 模式生效' },
