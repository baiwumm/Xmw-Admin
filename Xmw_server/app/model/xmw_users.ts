 ...
 * @LastEditors: Cyan
 * @LastEditTime: 2022-09-09 12:39:05
 */
'use strict';
...
module.exports = (app) => {
  ...
  const xmw_users = app.model.define('xmw_users', {
    user_id: { type: STRING(20), primaryKey: true, comment: '用户id' },
    ...
    jobs_id: { type: STRING(20), allowNull: false, comment: '岗位id' },
    org_id: { type: STRING(20), allowNull: false, comment: '组织id' },
    role_id: { type: STRING(20), allowNull: false, comment: '权限id' },
    ...
    update_time: { type: DATE, allowNull: false, comment: '创建日期' },
    founder: { type: STRING(20), allowNull: false, comment: '创建人' },
  });
