 ...
 * @LastEditors: Cyan
 * @LastEditTime: 2022-10-10 10:13:03
 */
...
export default class Internationalization extends BaseController {
    ...
    public async saveInternational() {
        ...
        try {
            ...
            if (id) {
                ...
                // 执行更新操作
                await this._update('XmwInternationalization', params, id).then(({ error }) => {
                    // 判断是否更新成功
    ...
    public async delInternational() {
        ...
        try {
            ...
            // 不存在子级则执行删除操作
            await this._delete('XmwInternationalization', id).then(({ error }) => {
                // 判断是否删除成功
                error ? this.resResult(-10, {}) : this.resResult(1, {});
            })
        } catch (error) {
